...
export class BackgroundTerminalManager {
    ...
    async start(args, sessionDir) {
        ...
        proc.stderr?.on("data", (data) => appendLog(data));
        proc.on("error", (err) => {
            record.status = "error";
...
${recent || "[No output captured yet]"}`;
     ...
     */
    async executeMaster(args, sessionDir) {
        switch (args.action) {
            case "run":
            case "start":
                return this.start({
            ...
            case "read":
                if (!args.id)
                    return "❌ Error: 'id' argument is required for reading logs.";
                return this.getLogs({
                    id: args.id,
                    lines: args.lines,
                    search: args.search,
            ...
            case "send":
                if (!args.id)
                    return "❌ Error: 'id' argument is required for sending input.";
                if (args.input === undefined)
                    return "❌ Error: 'input' argument is required.";
            ...
            case "kill":
                if (!args.id)
                    return "❌ Error: 'id' argument is required for stopping terminal.";
                return this.stop({
                    id: args.id,
                    signal: args.signal,
                    force: args.force,
            ...
            default:
                return `❌ Unknown action '${args.action}'. Supported actions: 'run', 'status', 'logs', 'input', 'stop', 'list'.`;
        }
    }
    getAllTerminals() {
...
//# sourceMappingURL=background-terminal.js.map