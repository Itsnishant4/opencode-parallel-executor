...
import { BackgroundTerminalManager, BackgroundRunArgsSchema, BackgroundStatusArgsSchema, BackgroundLogsArgsSchema, BackgroundInputArgsSchema, BackgroundStopArgsSchema, BackgroundTerminalMasterArgsSchema, } from "./background-terminal.js";
import { TerminalUI } from "./ui/terminal-ui.js";
import { WebDashboardServer } from "./ui/web-dashboard.js";
const hasBun = typeof Bun !== "undefined";
export const ParallelExecutorPlugin = async (pluginInput) => {
    ...
    const batchExecuteTool = tool({
        description: "Execute 5, 10, or more operations (shell commands, file reads, writes, edits) simultaneously in high-concurrency parallel lanes (default 10 parallel lanes). ALWAYS use this when running multiple commands (e.g. 5 or 10 tests/lints/builds) or reading multiple files in parallel.",
    ...
    const parallelExecuteTool = tool({
        description: "Execute multiple file reads, file writes, file edits, and shell commands in parallel batches with automatic dependency analysis, concurrency throttling, and rollback safety. Use for multi-step dependent operations.",
    ...
    const fastReadTool = tool({
        description: "MANDATORY file reader (< 0.05ms). Always use this tool (NOT bash cat/head) to inspect file content or specific line ranges (startLine, endLine) from zero-latency RAM cache.",
        args: {
        ...
        async execute(args, context) {
            ...
            try {
                if (startLine !== undefined && endLine !== undefined) {
                    ...
                    if (res)
                        return OutputCompactor.compactFile(res.content);
                }
                const content = await cache.get(targetPath);
                if (content !== null)
                    return OutputCompactor.compactFile(content);
    ...
    const fastWriteTool = tool({
        description: "MANDATORY file writer (< 1ms). Always use this tool (NOT bash echo/cat/tee) to create new files or completely overwrite existing files.",
        args: {
    ...
    const fastEditTool = tool({
        description: "MANDATORY in-place file editor (< 0.5ms). Always use this tool (NOT bash sed/awk/python) to replace exact substrings or lines within an existing file.",
        args: {
    ...
    const fastBashTool = tool({
        description: "Execute shell processes: build tools, package managers (pnpm/npm/yarn/bun), compilers, test runners, and git commands (< 1ms persistent shell). STRICT PROHIBITION: DO NOT use bash to read files (use 'read'), write files (use 'write'), edit files (use 'edit'), or search files (use 'glob'/'grep').",
        args: {
    ...
    const fastGlobTool = tool({
        description: "MANDATORY file finder (< 5ms). Always use this tool (NOT bash find/ls) to locate files by name or glob pattern across the workspace.",
        args: {
    ...
    const fastGrepTool = tool({
        description: "MANDATORY codebase text searcher (< 10ms). Always use this tool (NOT bash grep/rg) to find strings or regex patterns across project files.",
        args: {
    ...
    const multiEditTool = tool({
        description: "Transaction-safe multi-file and multi-location patch tool (< 2ms). Applies multiple file edits atomically with 2-phase commit and all-or-nothing rollback safety. Supports dryRun preview.",
    ...
    const outlineTool = tool({
        description: "Instant structural code symbol extractor (< 0.5ms). Extracts functions, classes, interfaces, types, and methods with line numbers across TypeScript/JS, Python, Go, and Rust without loading full file text into context.",
    ...
    const findReplaceTool = tool({
        description: "Ultra-fast project-wide find and replace (< 15ms). Searches candidate files matching a text or regex pattern and replaces occurrences across the entire repository. Supports dryRun preview.",
    ...
    const gitChangesTool = tool({
        description: "Zero-fork git status and diff inspector (< 3ms). Returns modified, added, deleted, untracked files and unified diffs in-process without spawning bash shells.",
    ...
    const verifyTool = tool({
        description: "Smart codebase auto-verifier (< 50ms). Detects TypeScript, Python, Rust, or Go project configuration and runs typechecks, linting, and compile checks concurrently in parallel lanes. Returns structured diagnostic error tables with file, line, and message.",
    ...
    const snapshotTool = tool({
        description: "Working tree checkpointing tool (< 1ms). Creates an in-memory snapshot of all dirty/modified/untracked files before risky edits or refactors. Allows instant rollback with undo without touching git stash.",
    ...
    const undoTool = tool({
        description: "Working tree atomic rollback tool (< 2ms). Restores all files back to a snapshot checkpoint, reverting modified files and removing any newly created files. Supports restoring specific snapshotId or latest checkpoint.",
        args: UndoArgsSchema,
        async execute(args, context) {
            const sessionDir = typeof context?.directory === "string" ? context.directory : undefined;
    ...
    const findReferencesTool = tool({
        description: "Cross-file symbol and reference locator (< 3ms). Discovers all definitions, import statements, call sites, and usages of a symbol across the entire workspace.",
    ...
    const backgroundRunTool = tool({
        description: "Launch long-running commands, dev servers (vite, next dev, nodemon), build daemons, test watchers, and background tasks in a detached background terminal (< 0.2ms). Returns control IMMEDIATELY so the main agent can continue doing other work concurrently without blocking.",
    ...
    const backgroundStatusTool = tool({
        description: "Inspect status of a specific background terminal (PID, uptime, exit code, log file) or list all active and recent background terminals in an overview table.",
    ...
    const backgroundLogsTool = tool({
        description: "Read live stdout/stderr logs from a background terminal. Supports tailing recent lines, pagination offset, regex/substring searching, and clearing buffer.",
    ...
    const backgroundInputTool = tool({
        description: "Send stdin input to an actively running background terminal process (e.g. typing responses to interactive prompts, typing 'rs' for nodemon restart, answering confirmations).",
    ...
    const backgroundStopTool = tool({
        description: "Terminate a background terminal process and its entire process tree safely using SIGTERM (graceful) or SIGKILL (force).",
    ...
    const backgroundTerminalMasterTool = tool({
        description: "Master background terminal controller for OpenCode. Run, check status, view logs, send stdin, or terminate background processes without blocking the main agent thread.",
    ...
    const dashboardTool = tool({
        description: "Interactive UI Dashboard for OpenCode (Terminal TUI, Desktop App, & Web Browser). Displays real-time background terminals, live streaming logs, RAM cache metrics, 10-lane concurrency monitor, and the 35+ accelerated tool catalog.",
        args: {
