...
export class FastBashRunner {
    ...
    persistentShell = PersistentShell.getInstance();
     ...
     */
    async tryInProcess(rawCommand, cwd) {
        ...
        const cleanArg = (arg) => arg.replace(/^["']|["']$/g, "");
        // 1. pwd
        if (cmd === "pwd") {
            return {
                stdout: effectiveCwd,
     ...
     */
    async execute(command, cwd, timeoutMs = 60000, abortSignal) {
    ...
    executeDirectSpawn(command, effectiveCwd, timeoutMs, abortSignal) {
        const t0 = Date.now();
        if (abortSignal?.aborted) {
...
//# sourceMappingURL=fast-bash.js.map