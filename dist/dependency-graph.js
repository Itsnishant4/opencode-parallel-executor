...
export function isSetupCommand(cmd) {
...
export function isVerificationCommand(cmd) {
    ...
    if (/^git\s+(checkout|switch|branch|status|diff|add|commit|log)\b/.test(normalized))
        return false;
    if (/^(mkdir|touch|rm|cp|mv|cat|echo|cd|pwd)\b/.test(normalized))
        return false;
...
export class DependencyGraph {
    static normalizeOperations(reads = [], writes = [], edits = [], commands = []) {
        const ops = [];
        // 1. Reads
     ...
     */
    static resolveFineGrainedDependencies(ops, baseDir = process.cwd()) {
        // Map each file to its previous mutation task ID (write/edit)
        const fileLastMutation = new Map();
        // Map each file to all active concurrent reader task IDs since last mutation
        const fileActiveReaders = new Map();
        const mutationTaskIds = [];
        const manifestMutationIds = [];
        const setupCommandIds = [];
        for (const op of ops) {
            const deps = new Set(op.dependsOn || []);
            if (op.type === "read") {
