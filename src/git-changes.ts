...
export class GitChangesInspector {
  ...
  constructor(defaultRootDir: string = process.cwd()) {
    this.defaultRootDir = defaultRootDir;
  }

  private resolveRepoDir(targetPath?: string): string {
    if (!targetPath) return this.defaultRootDir;
    const resolved = path.isAbsolute(targetPath) ? targetPath : path.resolve(this.defaultRootDir, targetPath);
    return resolved;
  }

  private parseStatusCode(x: string, y: string): { status: GitFileStatus["status"]; staged: boolean } {
    if (x === "?" && y === "?") {
    ...
    if (x === "M" || y === "M") return { status: "modified", staged: isStaged };
    if (x === "A" || y === "A") return { status: "added", staged: isStaged };
    if (x === "D" || y === "D") return { status: "deleted", staged: isStaged };
    if (x === "R" || y === "R") return { status: "renamed", staged: isStaged };

    return { status: "unknown", staged: isStaged };
  }

  async inspect(args: GitChangesArgs = {}, abortSignal?: AbortSignal): Promise<GitChangesResult> {
    ...
    if (statusOutput) {
      ...
      for (const line of lines) {
        ...
        if (args.staged === true && !staged) continue;
        if (args.staged === false && staged && x !== " " && y === " ") continue;
        ...
        if (staged) summary.stagedCount++;
        if (status === "modified") summary.modified++;
        else if (status === "added") summary.added++;
        else if (status === "deleted") summary.deleted++;
        else if (status === "untracked") summary.untracked++;
    ...
    if (includeDiff && summary.totalChanged > 0) {
      try {
        ...
        try {
          ...
          if (fileFilter) fallbackFlags.push("--", fileFilter);
          ...
          if (fileFilter) cachedFlags.push("--", fileFilter);
