...
export class FindReplaceEngine {
  ...
  public async executeFindReplace(
    args: {
      find: string;
      replace: string;
      isRegex?: boolean;
    ...
    const summaries: FileReplacementSummary[] = [];
    ...
    for (const relFile of candidateFiles) {
      ...
      if (relFromEffective.startsWith("..") || path.isAbsolute(relFromEffective)) continue;
    ...
    try {
      await Promise.all(
        Array.from(modifiedMap.entries()).map(async ([absPath, newContent]) => {
          if (hasBun) {
