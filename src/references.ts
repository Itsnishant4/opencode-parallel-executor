...
function makeWordRegex(symbol: string): RegExp {
...
export class CodeReferenceLocator {
  ...
  private classifyUsage(line: string, symbol: string): SymbolReferenceItem["kind"] {
    const trimmed = line.trim();
    const escaped = escapeRegex(symbol);

    // 1. Definition patterns
    if (
      new RegExp(`^(export\\s+)?(default\\s+)?(async\\s+)?(function|class|interface|type|enum)\\s+${escaped}\\b`).test(trimmed) ||
    ...
    if (
      new RegExp(`\\b${escaped}\\s*(?:<[^>]+>)?\\s*\\(`).test(trimmed) ||
  ...
  public async findReferences(
    ...
    if (!symbol) return "Error: symbol name is required.";
    ...
    const references: SymbolReferenceItem[] = [];
    const wordRegex = makeWordRegex(symbol);

    for (const match of searchRes.matches) {
      if (!wordRegex.test(match.content)) continue;
