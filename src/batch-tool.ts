...
export class BatchExecutor {
  ...
  public async runBatch(
    args: z.infer<z.ZodObject<typeof BatchExecuteArgsSchema>>,
    ...
    // 1. Normalize all inputs into a uniform task queue
    type NormalizedBatchTask = {
      index: number;
      id: string;
      type: "command" | "read" | "write" | "edit";
      command?: string;
    ...
    const taskQueue: NormalizedBatchTask[] = [];
    let seq = 1;
    ...
    const results: BatchItemResult[] = new Array(taskQueue.length);
    let queueIdx = 0;
    let hasFailure = false;

    const executeItem = async (task: NormalizedBatchTask): Promise<BatchItemResult> => {
  ...
  private formatReport(
    ...
    lines.push(
      `${icon} **batch_execute**: ${results.length} tasks completed in ${totalDurationMs}ms (${concurrency} parallel lanes) — ${succeeded} passed, ${failed} failed${skipped > 0 ? `, ${skipped} skipped` : ""}`
    );
    lines.push("");
    ...
    for (const r of results) {
      ...
      lines.push(
        `| ${r.index} | \`${r.type}\` | \`${targetClean}\` | ${statusTag} | ${r.durationMs}ms | ${outputSnippet || "(done)"} |`
      );
    }
