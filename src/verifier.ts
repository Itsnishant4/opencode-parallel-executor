...
export class CodebaseVerifier {
  ...
  public parseDiagnostics(rawOutput: string, checkName: string): DiagnosticItem[] {
    ...
    for (const line of lines) {
      ...
      if (m) {
        diagnostics.push({
          ...
          col: parseInt(m[2], 10),
          severity: m[3].toLowerCase() as "error" | "warning",
          code: m[5] || "",
          message: m[4].trim(),
        });
        continue;
  ...
  public async verify(args: VerifyArgs = {}, sessionDir?: string, abortSignal?: AbortSignal): Promise<string> {
    ...
    const results: VerificationCheckResult[] = await Promise.all(
