...
export class TaskExecutor {
  ...
  private static knownDirs = new Set<string>();
  private fileCache = FastFileCache.getInstance();
  private bashRunner: FastBashRunner;
   ...
   */
  private async snapshotFile(targetPath: string): Promise<void> {
    const resolved = this.resolvePath(targetPath);
    if (this.backups.has(resolved)) return;

    try {
      let content: string | null = await this.fileCache.get(resolved);
      if (content === null) {
        if (hasBun) {
   ...
   */
  private async readLinesStream(
    resolvedPath: string,
    startLine: number,
    endLine: number,
    showLineNumbers: boolean
  ): Promise<{ content: string; linesRead: number }> {
  ...
  private async executeRead(op: NormalizedReadOp): Promise<TaskResult> {
    ...
    try {
      ...
      let totalChars = 0;
      let lineMeta: Record<string, any> = {};
      ...
      if (startLine !== undefined && endLine !== undefined) {
        ...
        if (cached) {
          ...
          const { content: linesContent, linesRead } = await this.readLinesStream(
            resolvedPath,
            startLine,
            endLine,
            showLineNumbers
  ...
  private async executeWrite(op: NormalizedWriteOp): Promise<TaskResult> {
    const startTime = Date.now();
    const resolvedPath = this.resolvePath(op.path);
    ...
    try {
      ...
      if (hasBun) {
        ...
        await fs.writeFile(resolvedPath, op.content, (op.encoding as BufferEncoding) || "utf-8");
      }
  ...
  private async executeEdit(op: NormalizedEditOp): Promise<TaskResult> {
    const startTime = Date.now();
    const resolvedPath = this.resolvePath(op.path);
    ...
    try {
      ...
      if (!content.includes(op.oldText)) {
        throw new Error(
          `Target substring to replace was not found in ${op.path}. Ensure exact match.`
        );
      }
  ...
  private async executeCommand(op: NormalizedCommandOp): Promise<TaskResult> {
    ...
    const compactedOutput = OutputCompactor.compactCommand(
      op.command,
      res.stdout,
      res.stderr,
      res.exitCode
  ...
  private async executeTask(task: NormalizedOp): Promise<TaskResult> {
    switch (task.type) {
      case "read":
   ...
   */
  public async executeContinuousDAG(ops: NormalizedOp[]): Promise<ParallelExecuteResult> {
    ...
    for (const op of ops) {
      ...
      if (!dependentsMap.has(op.id)) dependentsMap.set(op.id, []);
    }
    ...
    await new Promise<void>((resolveComplete, rejectComplete) => {
      let activeWorkers = 0;
      ...
      const dispatch = () => {
        ...
        for (let i = 0; i < readyQueue.length; i++) {
          ...
          if (isCmd && activeCommands >= this.options.maxCommandConcurrency) continue;
          if (!isCmd && activeFiles >= this.options.maxFileConcurrency) continue;

          readyQueue.splice(i, 1);
          i--;

          if (isCmd) activeCommands++;
          else activeFiles++;
            ...
            .then(async (result) => {
              ...
              if (failed) {
                const shouldHalt =
                  ...
                  task.type === "write" ||
                  task.type === "edit" ||
                  (isCmd && !(task as NormalizedCommandOp).continueOnError);
