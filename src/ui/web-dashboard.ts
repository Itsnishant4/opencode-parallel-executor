...
export class WebDashboardServer {
  private static instance: WebDashboardServer | null = null;
  ...
  public static getInstance(rootDir?: string): WebDashboardServer {
   ...
   */
  public async start(preferredPort: number = 20888): Promise<{ url: string; port: number }> {
  ...
  private async bindServer(startPort: number): Promise<number> {
  ...
  private getCurrentState(port: number) {
    const bgMgr = BackgroundTerminalManager.getInstance(this.rootDir);
  ...
  public async saveStaticFile(port: number): Promise<string> {
